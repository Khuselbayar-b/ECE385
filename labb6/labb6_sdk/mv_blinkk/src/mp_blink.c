...
volatile uint16_t* switch_gpio_data = (uint16_t*)0x40010000;
volatile uint32_t* btns_gpio_data = (uint32_t*)0x40020000;
...
int main()
{
	...
	while (1+1 != 3)
	{
		...
		*/
		...
		if (*btns_gpio_data == 0x1) {
			*led_gpio_data = 0;
		}
		if (*btns_gpio_data == 0x2) {
			while (*btns_gpio_data == 0x2) {
				continue;
			}
